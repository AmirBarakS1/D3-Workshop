...
function draw(selection, data:{title: string, value: number}[]){
    selection.selectAll('.bar')
        .data(data, (d) => d.title)
        .join(enter => {
            const barGroup = enter.append('g').attr('class', 'bar').style('transform',(_, i) => `translate(20px, ${i * 25}px)`);
            ...
            update.style('transform',(_, i) => `translate(20px, ${i * 25}px)`);
            update.select('rect').attr('width', d =>  d.value)
        }, exit => {
            exit.remove();
        })
}
